...
namespace nanolog
{
    enum class LogLevel : uint8_t { INFO, WARN, CRIT }; //枚举类
	...
	typename std::enable_if < std::is_same < Arg, char const * >::value, NanoLogLine& >::type   //限制模板函数的参数类型,前面条件为真才继续执行
	...
	std::unique_ptr < char [] > m_heap_buffer;  //char[]  指char[] 数组
     ...
     */
    void initialize(GuaranteedLogger gl, std::string const & log_directory, std::string const & log_file_name, uint32_t log_file_roll_size_mb); //有锁版本
    void initialize(NonGuaranteedLogger ngl, std::string const & log_directory, std::string const & log_file_name, uint32_t log_file_roll_size_mb); //无锁版本
